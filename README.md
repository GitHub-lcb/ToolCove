...
速记与问题记录支持多设备**端到端加密**同步：服务器只保存密文，无法读取任何内容；匿名配对码入伙，无需账号。

- **零知识**：同步密码仅在本地派生加密密钥（PBKDF2 600k + AES-256-GCM 逐记录信封），遗忘密码 = 云端数据不可恢复（本地不受影响）。
- **自托管**：仓库内 `server/` 提供零依赖 Node 服务（HTTPS 反代部署见 [server/README.md](server/README.md)）。
- **Pro 门禁**：云同步为 Pro 功能（license feature：`cloud-sync`）。
...
云同步为显式开启的端到端加密同步：内容仅以密文形式存在于服务器。
