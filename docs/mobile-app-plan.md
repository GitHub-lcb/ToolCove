...
| 真机验证 | **未做，但已把成本降到一次点击** | 见 6.5.1 |

#### 6.5.1 真机验证为什么没做，以及怎么把它变得容易

本机**无法起模拟器**：虚拟化固件支持（`VirtualizationFirmwareEnabled=True`、SLAT 支持），
但 **Hyper-V / WHPX 未启用**（`HypervisorPresent=False`），启用需要管理员权限 + 重启——
不能为此重启开发机。`adb devices` 也为空，没有可用设备。

与其列一张"请你看四处"的清单让人手工核对，不如**让 App 自己跑一遍**：
设置 → 关于 → **原生能力自检**（`mobile/app/selfTest.js`）。一次点击逐项验证：

| 检查项 | 验的是什么 | 自动 |
|---|---|---|
| `store` | 写入后能否读回——**真机上最容易坏的一项**（桥存在但没实现 `load_data`/`save_data` 时，invoke 若不回退就全废） | ✅ |
| `http` | 请求本机 loopback 服务：同时验证原生 HTTP 通道与内置资源服务在监听 | ✅ |
| `tcp` | 对同一地址做 TCP 连通性检查：验证原生 socket 通道 | ✅ |
| `crypto` | 加解密往返，并检查**密文里不含明文**（Keystore 退化成直通是最危险的静默失败） | ✅ |
| `filePicker` | SAF 选择器能否拿到授权 URI | 需点选文件 |
| `sqlite` | 打开 .db 并列出表 | 需点选文件 |

设计约束：**只读或可回滚**（写一个临时键后立刻清掉，不碰用户数据）；
**不假装能测**（需要用户交互的标成 manual，不自动弹选择器）；**一项失败不中断后续**
（一次跑完拿完整报告）；跑完可**一键复制报告**（含构建时间、桥状态、逐项结论与耗时）。

16 条单测 + 5 条 E2E 覆盖：正常全过、存储不通时报出原因且其余项继续、
写入读不回、**加密退化成明文必须被发现**、往返不一致、HTTP 非 2xx、TCP 连不上、
拿不到页面地址、耗时记录、单项运行、用户取消、报告格式（含失败计数）。
...
### 6.8 把内置资源服务测起来（本轮）——抓到两个真缺陷

`AssetsServer` 是**启动路径**：它起不来或资源 404，表现就是白屏（而构建完全正常）。
以前只测了它的纯逻辑（`AssetPath`），**请求解析、状态码、响应头、并发、关闭都没测**——
因为绑死了 `AssetManager`。本轮把资源读取抽成可注入的 `AssetReader`，
于是可以在 JVM 里**起一个真服务、发真 socket 请求**（21 条新测试）。

抓到两个真缺陷：

1. **关闭带未读数据的 socket 会发 RST**。服务端原来只读**请求行**就响应并关闭，
   请求头还留在接收缓冲区里——内核因此发 RST，客户端读响应时撞上 `Connection reset`。
   浏览器因为 `Content-Length` 能提前停止读取，多数时候无感，但**并发拉资源时表现为偶发的
   资源加载失败**。修法：**读到空行为止**（把整个请求头读完）再响应。
   这是并发测试抓出来的（24 个并发请求里有 1 个报 Connection reset）。
2. **`describeLayout` 的启动诊断会误报**。它原来探测 `label-core.wasm` 这种确切文件名，
   而前端产物带**内容哈希**（`label-core-fHyB_A5r.wasm`）——真机上永远报 `missing`。
   **假警报比没有更糟**（会把人引到错误方向）。改为：入口文件按候选前缀探测（名字固定），
   其余按**目录列举**统计（js / css / wasm / locale 各几个）。

顺带把并发能力提上来：原来是 `newFixedThreadPool(4)` + `backlog 8`，24 个并发请求就有被重置的；
现在是 16 线程 + backlog 64（浏览器对单主机通常限 6 个连接，余量充足）。
每个响应都带 `Connection: close`（不能复用连接），所以并发连接数就是资源数——这个余量是必要的。

21 条测试覆盖：根路径/内容长度/MIME（含 `application/wasm`）/no-cache/404、
HEAD（body 空但长度真实）、OPTIONS 204、POST 405、**四种路径穿越都返回 404 且不泄露内容**、
只绑 loopback、**24 并发全部成功**、`stop` 后端口释放、重复 `stop` 不抛异常、
超长请求行被丢弃、连上就断开不崩、`describeLayout` 的三种情况。
...
- **可验证性**：装机后能**一键自检**原生能力并导出报告（设置 → 关于 → 原生能力自检），
  而不是靠人工按清单核对——开发机没有设备时这是唯一能闭环的手段。
