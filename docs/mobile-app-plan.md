...
| 工作台子模块 | overview / domain / iteration / requirement / release / task | overview / iteration / requirement / task | ⚠️ domain（领域）与 release（发布）未做，见 6.4 |
...
| 桌面端模块 | 手机端 | 说明 |
|---|---|---|
| `domain`（领域） | 未做 | 桌面端 DomainView 673 行；数据在 repository 的 `domains` 类别里，手机端可直接复用，缺的是界面 |
| `release`（发布） | 未做 | 桌面端 ReleaseView 674 行 + ReleasePackage 161 行；数据在 `release-pools` 类别 |
