...
const SENSITIVE_KEY = /(password|passwd|pwd|secret|token|api[_-]?key|access[_-]?key|private[_-]?key|credential|auth|session|cookie|phone|mobile|tel\b|telephone|id[_-]?card|idcard|identity|ssn|passport|bank|card[_-]?no|account|email|mail\b|address|addr\b|salary|密码|手机|电话|身份证|邮箱|地址|银行卡|账号|薪)/i;
